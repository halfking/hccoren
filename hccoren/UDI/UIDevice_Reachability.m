...
+ (NSString *) stringFromAddress: (const struct sockaddr *) address
{
	if (address && address->sa_family == AF_INET)
    {
		...
		return [NSString stringWithFormat:@"%@:%d",
                [NSString stringWithUTF8String:inet_ntoa(sin->sin_addr)], ntohs(sin->sin_port)];
	}
	...
	if (success) {
		...
		while (cursor != NULL) {
			...
			if (cursor->ifa_addr->sa_family == AF_INET && (cursor->ifa_flags & IFF_LOOPBACK) == 0)
			{
				...
				if ([name isEqualToString:@"en"] || [name isEqualToString:@"en0"])  // Wi-Fi adapter -- was en0
                {
					ret = [NSString stringWithUTF8String:inet_ntoa(((struct sockaddr_in *)cursor->ifa_addr)->sin_addr)];
                    break;
                }
			}
