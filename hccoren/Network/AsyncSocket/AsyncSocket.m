...
#define WRITEQUEUE_CAPACITY 10           // Initial capacity
#define READALL_CHUNKSIZE	256         // Incremental increase in buffer size
#define WRITE_CHUNKSIZE    (1024 * 8)   // Limit on size of each write pass
...
- (id)initWithData:(NSData *)d timeout:(NSTimeInterval)t tag:(long)i msg:(NSString*)msg;
#ifdef TRACK_SOCKET
- (NSString *)packetContent;
#endif
@end
...
- (id)initWithData:(NSData *)d timeout:(NSTimeInterval)t tag:(long)i msg:(NSString*)msg
{
    if((self = [super init]))
    {
        buffer = PP_RETAIN(d);//[d retain];
#ifdef TRACK_SOCKET
        //        NSData * data = [NSData dataWithCompressedData:d];
        stringTemp = [msg copy];// [[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding];
        DLog(@"sc:%@",stringTemp);
#endif
        timeout = t;
        tag = i;
...
- (int)writeQueueCount
{
    if(theWriteQueue && theWriteQueue.count>0)
    {
        return (int)theWriteQueue.count;
    }
    return 0;
}
...
- (id)initWithDelegate:(id)delegate userData:(long)userData
{
    if((self = [super init]))
    {
        ...
        if(theReadQueue!=nil) PP_RELEASE(theReadQueue);// [theReadQueue release];
        theReadQueue = [[NSMutableArray alloc] initWithCapacity:READQUEUE_CAPACITY];
        theCurrentRead = nil;
        theReadTimer = nil;
        
        if(partialReadBuffer) PP_RELEASE(partialReadBuffer);// [partialReadBuffer release];
        partialReadBuffer = [[NSMutableData alloc] initWithCapacity:READALL_CHUNKSIZE];
        
        if(theWriteQueue!=nil) PP_RELEASE(theWriteQueue);// [theWriteQueue release];
        ...
        theRunLoopModes = PP_RETAIN([NSArray arrayWithObject:NSDefaultRunLoopMode]);
    }
    return self;
}
...
{
    [self close];
    PP_RELEASE(theReadQueue);
    PP_RELEASE(theWriteQueue);
    PP_RELEASE(theRunLoopModes);
    PP_RELEASE(partialReadBuffer);
    //
...
- (BOOL)moveToRunLoop:(NSRunLoop *)runLoop
{
    ...
    PP_RETAIN(theReadTimer);
    PP_RETAIN(theWriteTimer);
    //	[theReadTimer retain];
    ...
    PP_RELEASE(theReadTimer);
    PP_RELEASE(theWriteTimer);
...
- (BOOL)addRunLoopMode:(NSString *)runLoopMode
{
    ...
    PP_RELEASE(theRunLoopModes);
    theRunLoopModes = PP_RETAIN(newRunLoopModes);//[newRunLoopModes retain];
...
- (BOOL)removeRunLoopMode:(NSString *)runLoopMode
{
    ...
    NSMutableArray *newRunLoopModes = PP_AUTORELEASE([theRunLoopModes mutableCopy]);//[theRunLoopModes mutableCopy] autorelease];
...
{
    return [self connectToHost:hostname onPort:port withTimeout:CONNECT_TIMEOUT error:errPtr];
}
...
- (BOOL)createSocketForAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
    ...
    if(pSockAddr->sa_family == AF_INET)
    {
        ...
        //        CFSocketSetSocketFlags(theSocket4, SO_REUSEADDR);
    }
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
    if(newNativeSocket)
    {
        ...
        AsyncSocket *newSocket = PP_AUTORELEASE([[[self class] alloc] initWithDelegate:theDelegate]);
        ...
        else
        {
            ...
            if([newSocket attachStreamsToRunLoop:runLoop error:nil])
                if([newSocket configureStreamsAndReturnError:nil])
...
- (void)doSocketOpen:(CFSocketRef)sock withCFSocketError:(CFSocketError)socketError
{
    ...
    //更改，为了能够快速重联服务器 2013-01-04
    CFSocketSetSocketFlags(sock, SO_REUSEADDR);
...
- (void)maybeScheduleDisconnect
{
    ...
    else if(theFlags & kDisconnectAfterWrites)
    {
        ...
        else if(theCurrentWrite==nil)//force to send? addby huangxt 2013-05-09
        {
            [self maybeDequeueWrite];
        }
    }
...
- (void)maybeDequeueRead
{
    ...
    if((theCurrentRead == nil) && (theReadStream != NULL))
    {
        if([theReadQueue count] > 0)
        {
            ...
            theCurrentRead = PP_RETAIN([theReadQueue objectAtIndex:0]);
...
#pragma mark Writing
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag msg:(NSString *)msg
{
#if DEBUG_THREAD_SAFETY
    [self checkForThreadSafety];
#endif
    ...
    AsyncWritePacket *packet = [[AsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag msg:msg];
...
{
    [self writeData:data withTimeout:timeout tag:tag msg:nil];
}
...
- (void)maybeDequeueWrite
{
    ...
    if((theCurrentWrite == nil) && (theWriteStream != NULL))
    {
        if([theWriteQueue count] > 0)
        {
...
#endif
            // Dequeue the next object in the write queue
            theCurrentWrite = PP_RETAIN([theWriteQueue objectAtIndex:0]);
            ...
            {
#ifdef TRACK_SOCKET
                NSLog(@"writing..333....%@..",[theCurrentWrite packetContent]);
#endif
                // Start time-out timer
                if(theCurrentWrite->timeout >= 0.0)
                {
#ifdef TRACK_SOCKET
                    NSLog(@"writing..333.loop..timeout:%f.....",theCurrentWrite->timeout);
#endif
...
{
#ifdef TRACK_SOCKET
    NSLog(@"theflags:%d kSocketCanAcceptBytes:%d",theFlags,kSocketCanAcceptBytes);
#endif
...
- (void)doSendBytes
{
    ...
    BOOL canAccept = NO;
    if(!done && !error)
        canAccept = [self canAcceptBytes];
#ifdef TRACK_SOCKET
    NSLog(@"done:%d error:%d canacceptbytes:%d",done,error,canAccept);
#endif
    while (!done && !error && canAccept)
    {
...
#ifdef TRACK_SOCKET
        NSLog(@"writing..666.....bytes:%d  send:%d",bytesRemaining,bytesToWrite);
#endif
        UInt8 *writestart = (UInt8 *)([theCurrentWrite->buffer bytes] + theCurrentWrite->bytesDone);
#ifdef TRACK_SOCKET
        NSLog(@"writing..666....content:%@",[theCurrentWrite packetContent]);
#endif
        // Write
...
#ifdef TRACK_SOCKET
            NSLog(@"writing..packe done:%d",(int)done);
#endif
        }
    }
#ifdef TRACK_SOCKET
