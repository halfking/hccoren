...
//- (HTTPMessage *)prepareUniRangeResponse:(UInt64)contentLength;
//- (HTTPMessage *)prepareMultiRangeResponse:(UInt64)contentLength;
...
- (id)initWithAsyncSocket:(GCDAsyncSocket *)newSocket configuration:(HTTPConfig *)aConfig
{
    if ((self = [super init]))
    {
        ...
        remainBody_ = nil;
        tmpUploadFileHandle_ = nil;
        requestBoundry_ = nil;
        userAgent_ = nil;
        parametersAddition_ = [NSMutableDictionary new];
...
- (void)dealloc
{
    ...
    PP_RELEASE(parametersAddition_);
...
- (NSDictionary *)parametesAddtion
{
    return parametersAddition_;
}
/**
...
- (void)replyToHTTPRequest
{
    ...
    {
        resource = [[FileResource alloc] initWithConnection:self delegate:config.server.fileResourceDelegate];
        //		resource.delegate = server.delegate;
        [resource handleRequest:request];
        return;
    }
    //	if([VDCFileResource canHandle:request])
    //    {
    //        VDCFileResource * cResource = [[VDCFileResource alloc]initWithConnection:self];
    //        [cResource handleRequest];
    //        return;
    //    }
    
    // Extract requested URI
    NSString *uri = [self requestURI];
    NSLog(@"local request:%@",uri);
...
- (void)sendResponseHeadersAndBody
{
    [self sendResponseHeadersAndBody:httpResponse method:request.method];
}
- (void)sendResponseHeadersAndBody:(NSObject<HTTPResponse> *)cResponse method:(NSString *)method
{
    ...
    if ([method isEqualToString:@"HEAD"] || isZeroLengthResponse)
    {
        ...
        {
            NSLog(@"wirte:%@",[[NSString alloc]initWithData:responseData encoding:NSUTF8StringEncoding]);
        }
...
{
    HTTPMessage * response = [[HTTPMessage alloc]initResponseWithStatusCode:302 description:nil version:HTTPVersion1_1];
...
- (void)sendString:(NSString*)text mimeType:(NSString*)mimeType
{
    ...
    [response setHeaderField:@"Cache-Control" value:@"private, max-age=0, must-revalidate"];
    ...
    [asyncSocket writeData:[content dataUsingEncoding:NSUTF8StringEncoding] withTimeout:TIMEOUT_WRITE_ERROR tag:HTTP_FINAL_RESPONSE];
...
- (NSString *)filePathForURI:(NSString *)path allowDirectory:(BOOL)allowDirectory
{
    ...
    if([relativePath hasSuffix:@"/"])
    {
        ...
        fullPath = [NSString stringWithFormat:@"%@/%@", documentRoot, completedRelativePath];
    }
    else
    {
        fullPath = [NSString stringWithFormat:@"%@/%@", documentRoot, relativePath];
    }
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
    ...
    //    if(self.request.url)
    //    {
    //        NSLog(@"http request:%@",[self.request.url absoluteString]);
    //    }
    
    // Override me to provide custom responses.
    NSString *filePath = nil;
    
    //    if([[VDCManager shareObject] canHandleUrl:path])
    //    {
    //        BOOL isDirTemp = NO;
    //        filePath = [[VDCManager shareObject]getFilePathForLocalUrl:path];
    //        if(![[NSFileManager defaultManager]fileExistsAtPath:filePath isDirectory:&isDirTemp])
    //        {
    //            filePath = [[VDCManager shareObject]getTempFilePathForLocalUrl:path];
    //        }
    ...
    if(fileHandlerList_)
    {
        for (id<fileHandlerProtocol> item in fileHandlerList_) {
            handle = [item handleResponse:path];
            if(handle)
            {
                break;
            }
...
- (void)prepareForBodyWithSize:(UInt64)contentLength
{
    ...
    if(![request.method isEqualToString:@"POST"]) return;
    
    NSLog(@"*********** perpare For BodySize");
    // init body info
    NSDictionary* header = (NSDictionary*)request.allHeaderFields;
    PP_RELEASE(userAgent_);
    userAgent_ = PP_RETAIN([header objectForKey:@"User-Agent"]);
    ...
    [self parseUploadHeader:request.body];
    
}
- (void)parseUploadHeader:(NSData *)data
{
    if(![request isHeaderComplete]) return;
    
    //    NSString* progress = [NSString stringWithFormat:@"%f", 1.0 - remain * 1.0 / bodyLength];
    //    NSDictionary * dic = [NSDictionary dictionaryWithObjectsAndKeys:progress,@"progress",fileName?fileName:@"",@"file", nil];
    //    [[NSNotificationCenter defaultCenter] postNotificationName:HTTPUploadingProgressNotification object:dic];
    NSString * EOL = @"\015\012";
    if(data&& data.length > requestBoundry_.length + [EOL length]){
        NSLog(@"parsing multipart header");
        
        // check boundary
        NSRange range = NSMakeRange(0, [requestBoundry_ length] + [EOL length]);
        NSString *bHead = [NSString stringWithUTF8String:(const char *)[[data subdataWithRange:range] bytes]];
        if (![bHead isEqualToString:[NSString stringWithFormat:@"%@%@", requestBoundry_, EOL]])
        {
        ...
        if ([userAgent_ isMatchedByRegex:@"MSIE .* Windows "]
        ...
        tmpUploadFileHandle_ = PP_RETAIN([NSFileHandle fileHandleForWritingAtPath:tmpName]);
        
        if(!parametersAddition_)  parametersAddition_ = [NSMutableDictionary new];
        [parametersAddition_ setObject:fileName forKey:key];
        if([key isEqualToString:@"filename"]==NO)
        {
            [parametersAddition_ setObject:fileName forKey:@"filename"];
            
        }
        [parametersAddition_ setObject:tmpName forKey:@"tmpfilename"];
        
        PP_RELEASE(bodyHeader);
        ...
        [self processBodyData:fileContent];
    }
}
/**
...
- (void)processBodyData:(NSData *)postDataChunk
{
    ...
    NSLog(@"*********** preocess body data");
    if (nil == tmpUploadFileHandle_)
    {
        [self parseUploadHeader:postDataChunk];
        return;
    }
    static NSString* deol = @"\015\012";
    NSString *terminator = [NSString stringWithFormat:@"%@%@", deol, requestBoundry_];
    NSMutableData *data = [[NSMutableData alloc] init];
    if (remainBody_)
        [data appendData:remainBody_];
    ...
    long taillen = [data length] + bytes - candidate;
    ...
    if (NULL != contentEnd)
    {
        ...
        [tmpUploadFileHandle_ writeData:content];
        //        PP_RELEASE(tmpUploadFileHandle_);
        //        PP_RELEASE(requestBoundry_);
    }
    else
    {
        ...
        PP_RELEASE(remainBody_);
        remainBody_ = PP_RETAIN([data subdataWithRange:range]);
        
        [tmpUploadFileHandle_ writeData:content];
    }
    PP_RELEASE(data);
    [self sendProgress:requestContentLengthReceived contentLength:requestContentLength];
}
...
- (void)finishBody
{
    ...
    if(remainBody_)
    {
        [tmpUploadFileHandle_ writeData:remainBody_];
        PP_RELEASE(remainBody_);
    }
    PP_RELEASE(tmpUploadFileHandle_);
    PP_RELEASE(requestBoundry_);
...
- (void)sendUploadCompletedMsg
{
    NSString * fileName = parametersAddition_?[parametersAddition_ objectForKey:@"filename"]:nil;
    NSDictionary * dic = [NSDictionary dictionaryWithObjectsAndKeys:@(1),@"progress",fileName?fileName:@"",@"file", nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:HTTPUploadingFinishedNotification object:dic];
}
- (void)sendProgress:(UInt64)readBytes contentLength:(UInt64)totalBytes
{
    ...
    NSString* progress = [NSString stringWithFormat:@"%f", readBytes * 1.0 / totalBytes];
    NSString * fileName = parametersAddition_?[parametersAddition_ objectForKey:@"filename"]:nil;
    NSDictionary * dic = [NSDictionary dictionaryWithObjectsAndKeys:progress,@"progress",fileName?fileName:@"",@"file", nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:HTTPUploadingProgressNotification object:dic];
}
...
//
//    NSString* progress = [NSString stringWithFormat:@"%f", 1.0 - remain * 1.0 / bodyLength];
//    NSString * fileName = resource?[resource lastFileName]:nil;
//    NSDictionary * dic = [NSDictionary dictionaryWithObjectsAndKeys:progress,@"progress",fileName?fileName:@"",@"file", nil];
//    [[NSNotificationCenter defaultCenter] postNotificationName:HTTPUploadingProgressNotification object:dic];
//
//    if (readLength > 0)
...
#pragma mark - handle items
- (void)addHandleItem:(id<fileHandlerProtocol>)item
{
    if(!fileHandlerList_) fileHandlerList_ = [NSMutableArray new];
    [fileHandlerList_ addObject:item];
}
- (void)removeHandleItem:(id<fileHandlerProtocol>)item
{
    if(fileHandlerList_)
    {
        [fileHandlerList_ removeObject:item];
    }
}
- (void)clearHandleItem
{
    [fileHandlerList_ removeAllObjects];
}
